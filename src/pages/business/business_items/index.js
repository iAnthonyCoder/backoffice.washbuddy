...
import { fetchData } from 'src/store/apps/item'
...
const Business_itemList = ({ apiData }) => {
  ...
  const removeRecord = async (record) => {
    try {
      setIsDeleting(true)
      await business_itemService.delete(record.id)
    ...
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Card>
          ...
          <TableHeader title='Items' value={value} handleFilter={handleFilter} toggle={toggleAddUserDrawer} />
          <Divider />
    {console.log(store)}
          <DataGrid
            autoHeight
...
Business_itemList.acl = {
  action: 'manage',
  subject: 'business'
}
export default Business_itemList
